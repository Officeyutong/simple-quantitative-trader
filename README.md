...
- 市价单/限价单风险预览；
- `trading_enabled`、账户级风险限额，以及数据库管理的策略资本、亏损和换手门控；
- 幂等订单意图、风险决策和提交记录；
...
“下载任务”页面通过服务端分页展示数据库中的全部历史行情和历史汇率任务，包括真实运行状态、
IBKR worker 状态、队列位置、证券/汇率对与交易时段、完整请求范围、游标进度、已完成分片、
重试次数和最近错误。活动任务可在该页面取消；`data.jobs` 接受 `page` 与 `page_size`
并返回 `total_items`、`total_pages` 和全局队列摘要，因此翻页不会丢失当前 worker 信息。

“策略绩效”页面可选择策略和初始资金，分开展示已实现净损益、未实现损益和完整
盯市总损益，并展示收益率、已实现权益最大回撤、胜率、Sharpe、Sortino、交易统计及历史绩效
快照。绩效只归因于该策略执行层产生并实际成交的订单；持仓快照、实时估值、成交、
佣金或 FX 任一不完整时，总损益会明确显示为“不可计算”，不会拿已实现结果冒充总
损益。同类历史问题按原因、币种和时间范围汇总，逐笔审计明细默认折叠且单次响应
最多返回 100 条。点击“修复历史数据”会先执行一次完整 IBKR 订单/成交对账，再为
缺少成交时汇率的币种创建可去重、可重试的 1 分钟 MIDPOINT 下载任务；任务完成后
页面自动重新计算。时间均按浏览器本地时区显示。
...
cargo run -- --config config/local.toml backtest run \
  ...
  --short-window 1 --long-window 2 \
  --quantity 1 --short-target-quantity 0 \
...
timeframe，并强制使用该策略执行配置中的多头目标、空头目标和 `allow_short`。
执行配置缺失时 Web 不允许运行回测。需要测试另一只证券时，应创建新的策略配置。
Parquet 历史回测不允许用其他周期冒充策略绑定周期；`5s` 策略会下载真实 5 秒历史
Bar，并使用这些数据回测。
...
回测采用与自动执行一致的目标仓位语义：`buy` 调整到多头目标；`sell` 在禁止做空时
调整到 0，在允许做空时调整到配置的非正空头目标。CLI/RPC 的兼容字段 `quantity`
表示多头目标仓位，不再表示每个信号都追加的数量；临时回测可以再传
`short_target_quantity` 和 `allow_short`。当前仓位与目标异号时，模拟器先在下一根
Bar 开盘平到 0，并在确认归零后终止该旧目标，不会根据原信号自动建立反向仓位。
只有归零后新产生、仍然明确指向同一反向方向且足够新鲜的 `buy`/`sell` evaluation
才能创建新的反向目标，并按普通新开仓路径重新接受下一根 Bar 撮合、资金与成本处理；
`hold` 不会保留或重启旧反向意图。回测尚未模拟 IBKR 保证金、借券可用性和强平；
允许做空只验证策略与目标仓位逻辑，不能当作真实做空购买力证明。
...
行情状态为 `fresh`；缺失、失败或过期行情都会阻止风险增加订单。基于当前会话持仓
验证的严格平仓只可旁路这项开仓行情门控，全局交易开关、交易控制/紧急停止和交易
日历仍然生效。成交价 tick 会聚合成 DuckDB 中的一分钟 OHLC，
...
绩效按 strategy action 归因，报告已实现毛/净 PnL、未实现 PnL、完整盯市总 PnL、
佣金、换手率、胜率、已实现权益最大回撤、Sharpe、Sortino、每日已实现权益和可选
基准超额收益。
启用策略会按 `[monitoring]` 周期保存快照；旧快照保留“仅已实现”口径，不会被
误标为完整盯市结果。告警持久化 IBKR/对账异常、失败或延迟行情、Unknown 订单、
失败策略 action 和绩效快照错误。

佣金来自 IBKR 的实际 `CommissionReport`，不是程序按固定费率推算；非基础币种的
历史成交与佣金使用成交时已保存且仍在新鲜期限内的 FX 报价换算，不会在每次生成报告
时用当前汇率重估。缺少成交时汇率的旧成交会明确标记为数据不完整，并可从策略绩效页
创建 IBKR 历史 FX 修复任务。历史分钟 Bar 以收盘时刻入库，成交只能使用此前已经完成
的报价，避免未来数据。缺少成交明细的订单只通过 IBKR 对账恢复；若 IBKR 已不再返回
该历史成交，则必须依据 Activity Statement/Flex Report 补录，系统不会从 `Filled Size`
猜造成交价。累计佣金会随成交增加。“交易成本”页面可在 DuckDB 中维护每笔固定费、每股
...
强度。达到配置的最少平仓交易数后，若佣金/正毛利润超过上限，成本绩效门控只阻止
后续开仓或加仓，不会自动关闭整套执行配置，也不会阻止已有仓位减仓或平仓。
...
并写入 DuckDB 的 `fx_rates`，同时追加到 `fx_rate_history` 供已实现损益按成交时点
折算；该刷新周期必须小于
...
- `moving_average_cross_v2`：可选 1 分钟或 5 秒、SMA/EMA；真实交叉后会在有限
  窗口内等待均线差、ATR 与趋势过滤达标，再执行连续确认和冷却控制；EMA、候选
  交叉、确认进度、冷却进度与已发布方向按 `state_version = 3` 持久化，不会在每个
  滑动窗口重新初始化。已经发布方向后的原始反向交叉会立即产生
  `target_intent = "flatten_only"` 的保护信号：它只把原方向仓位降到 0，不会直接
  反手；随后若反向方向继续成立，必须重新通过均线差、ATR、趋势过滤、连续确认和
  冷却，才会产生新的 `target_intent = "directional"` 入场信号。停机补算时的旧信号
  不会污染已发布方向；穿过均线完全相等点的反转也能识别。

升级到数据库 schema 37 时，旧版 `moving_average_cross_v2` 状态无法安全解释。迁移
会清空其旧运行状态、暂停正在运行的 V2 策略，并禁用对应自动执行。请先核对实际持仓、
活动订单和策略参数，再明确恢复策略并重新启用执行；迁移不会替操作者猜测旧信号是否
已经成交。
...
按 `strategy_id` 运行时默认冻结并模拟当前策略的信号强度/往返成本安全门槛，以及
佣金/已完成周期正毛利润门控；也可选择 `fees_only` 只扣费用。减仓和平仓在两种环境中
都绕过成本门控。该模式不模拟策略风险预算、账户状态、行情新鲜度、活动订单冲突或
交易日历门控。
...
拒绝，而不是低估敞口。经当前持仓验证的严格平仓可以旁路这项开仓检查；无法折算时
不会伪造 1:1 汇率，风险审计以 `CLOSE_ONLY_ALLOWED` 明确标记旁路，记录中的
`estimated_notional = 0` 只是“未参与开仓名义额限制”的占位值，不能解释为真实名义
金额。
...
订单数、账户 PnL 和价格偏离。账户或持仓数据过期时禁止开仓；经当前持仓验证的严格
平仓只旁路持仓、敞口、亏损、下单速率和行情/FX/PnL 等开仓输入限制。全局
`trading_enabled`、交易控制/紧急停止和 IBKR 交易日历仍然生效。当前 IBKR 持仓快照成功
完成后，即使
账户完全空仓、没有任何 `positions_current` 行，也会按“已确认的零持仓”参与风控；
尚在同步或超过 `max_account_data_age_seconds` 的快照仍会阻止开仓。
每次持仓订阅及自动重试都有独立代际 ID；只有当前代际完整收到 PositionEnd 后才会
标记为就绪，旧订阅迟到的 Started、Position、PositionEnd、heartbeat 或结束事件均
被忽略。heartbeat 也会先确认 IBKR 连接仍然存在。schema 35 另外保存最近一次完整
持仓快照的完成时间：heartbeat 只能续租，不能把“没有持仓行”误当成成交后的新空仓
证明。若本地成交先于 IBKR 持仓更新到达，策略会保留信号等待持仓流追平，避免按旧
仓位重复下到同一目标。多笔手工平仓还会原子占用剩余平仓额度，合计不得穿越零点。
方向信号的目标快照独立于一次订单尝试持久化：普通开仓失败不会盲目追逐陈旧信号，
但保护性的严格减仓若因闭市、临时数据不可用、明确拒绝或撤单未完成，会把目标及
`next_attempt_at` 独立持久化；活动订单结束且持仓流追平后，调度器使用新的 action、
幂等键和合成 carrier 持续重试，无需等待新的 evaluation 或 `hold` Bar，直到持仓达到
目标或目标被新的明确方向信号取代。多空反转不会提交一笔穿零订单：旧 desired target
只获准把仓位归零，持仓流确认后即标记完成，不再用原信号自动执行第二腿。只有之后
新产生、明确且足够新鲜、与反向方向一致的 `buy`/`sell` evaluation 才能建立反向仓位，
并重新通过行情、成本、账户与风险等全部开仓门控；`hold` 不提供开仓授权。每条策略
订单在创建 order intent 的同一个存储临界区内还会重新核对 active desired target、
执行配置以及最新持仓到目标的方向和数量；认领后发生的手工成交不能把旧平仓数量变成
反向开仓。最终授权到 IBKR 返回提交结果之间还与策略评估、启停和执行配置修改共用同一
提交协调闸：新信号先取得协调闸时旧请求会被最终授权拒绝，旧请求先进入提交点时新信号
会等待券商结果后再接管。desired target 因新信号、停用执行、配置替换或过期而失效时，
已知且仍活动的关联券商订单会先经对账再自动请求撤销；从旧 schema 升级、尚无 desired
target 行的遗留策略订单也按失效订单处理。结果未知的 intent 仍保持阻断并要求对账，
系统不会发送补偿反单。

Web“交易成本”页同时维护独立的策略风险控制。每个执行策略默认获得可编辑的数据库
配置：策略资本 100000 基础币种、目标持仓不超过 1 倍资本、滚动 24 小时已实现净
亏损不超过 2%、最多连续 3 个净亏损周期、滚动 24 小时最多完成 10 个交易周期、
滚动换手不超过 10 倍资本。除策略资本外填 0 可关闭单项限制。触线或统计不完整只
阻止风险增加；经当前持仓验证的严格减仓和平仓只绕过这些策略级开仓阈值，仍须通过
全局 `trading_enabled`、交易控制/紧急停止和 IBKR 交易日历。复位累计统计不会清除客观的
滚动 24 小时窗口；部分平仓损失会立即计入，存在归因持仓、未决订单或处理中 action
时不能复位。复位说明与时间保存在 DuckDB。当前阈值、实时统计及触线原因会
同时显示在“交易成本”和“策略运行状态”页面。
策略资本的币种随数值一同保存在数据库；更改 `[risk].base_currency` 不会重新解释
旧金额。升级到 schema 34 后，旧版无币种的风险控制会安全地阻止启用执行及新开仓：
先暂停策略，在“交易成本 → 策略风险控制”核对资本金额并保存一次，再重新启用。
旧版 RPC v2 客户端在显式保存风险控制时可以省略 `capital_currency`；服务端会把本次
提交绑定到当前已校验的 `[risk].base_currency`。查询响应中的 `capital_currency` 才是
该笔资本的权威单位，旧记录会返回 `null`，不会被 Web 猜成当前币种。
历史空头只接受成交对应 action leg 当时保存的负目标作为授权；后来修改
`allow_short` 不会改变旧交易的绩效和风险统计。
...
`healthy`。
IBKR completed-order 的 `Filled Size` 会单独持久化并参与恢复；对账不会把已经
Filled/Cancelled/Rejected 的本地订单降回 Submitted，本地成交明细覆盖请求数量时也
会把订单恢复为 Filled。
