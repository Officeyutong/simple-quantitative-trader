...
Storage Writer actor。数据库最新 schema 为 37：除账户、行情、策略、订单、成交、
风控和绩效数据外，还包含 5 秒 Bar、订单剩余数量/最近成交价/`why_held`/
market-cap price、`broker_order_events` 状态事件审计、策略风险预算、FX 历史，以及
支持正常/扩展和单日多区间的 IBKR 交易日历缓存。`strategy_runtime_states` 按策略
保存带版本的 JSON 运行状态、修订号和最后转换 Bar；状态与策略 evaluation 在同一
事务提交。已实现损益按成交时已知的历史 FX 折算，当前 FX 只用于实时风险与未实现
估值。历史绩效修复复用持久化 data job 队列下载 IBKR `CASH` 合约的 1 分钟
`MIDPOINT`；报价以分钟收盘时刻写入 `fx_rate_history`，不能被同一分钟更早的成交使用。
修复入口在创建 FX 任务前先进行完整订单/成交对账，且绝不从订单累计成交数量推造缺失的
成交价。持仓订阅租约带独立 `subscription_id`，旧订阅的迟到事件不能把新快照提前标记
为可用；完整快照完成时间与 heartbeat 租约分开保存，本地成交未被持仓流反映前不会
再次计算目标仓位；方向信号的目标快照与每次订单尝试分开审计，保护性减仓失败后可
由持久目标调度新的 action 和幂等键重试，不依赖新的 evaluation 或 `hold` Bar。
跨零目标只授权归零，确认后终止旧 desired target；只有后续新鲜、明确且与反向方向
一致的 `buy`/`sell` evaluation 才能重新经过全部开仓门控。schema 37 同时把
`moving_average_cross_v2` 升级到状态版本 3；迁移会重置旧 V2 状态、暂停运行中的
V2 并禁用其自动执行，等待人工核对后恢复。策略资本也保存明确币种，修改 daemon
基础币种不会静默改变既有预算含义。自动订单在 order intent 落库的同一存储临界区
复核 action provenance、订单类型/时段语义与最新持仓目标；策略评估、启停和执行配置
修改还与最终授权至 IBKR 返回提交结果的区间共用协调闸，关闭认领到券商提交之间的
本地竞态。已提交订单的 desired target 后续失效时先对账再自动撤销；没有 desired 行的
旧 schema 遗留订单同样撤销，未知结果则保持阻断而不猜测或补偿。

当前回测与自动执行共享多头/空头目标仓位语义，并在下一根 Bar 开盘撮合。已保存策略
强制使用其执行配置中的多头目标、空头目标与 `allow_short`；异号目标只执行归零并
丢弃旧反向意图，后续必须由新鲜、明确且同方向的 `buy`/`sell` evaluation 创建新的
反向目标，`hold` 不授权开仓。回测与实时成本门控共享数据库费用模型，统一计算每笔固定费、
每股费、比例费、最低费、卖出税费、点差和滑点；已保存策略使用其绑定模型，临时回测
显式引用 `cost_model_id`。策略绑定回测默认冻结当前门控配置和实际佣金 P90，同时模拟
信号强度安全门槛与路径依赖的佣金/已完成周期毛利润门控；严格减仓绕过，`fees_only`
模式只扣费用。模拟器不包含 IBKR 保证金、借券、利息和强平模型，也不模拟账户、行情
新鲜度、订单冲突、交易日历或下述策略风险门控。实时绩效仍使用 IBKR 实际
`CommissionReport`。策略资本、滚动 24 小时净亏损、连续亏损、交易次数
和换手上限由数据库中的策略风险控制独立维护；这些策略级开仓门控不锁住经当前持仓
验证的严格减仓和平仓，但全局交易开关、交易控制/紧急停止和交易日历仍然生效。
...
- `buy`/`sell` 映射为多头/空头目标仓位；禁止做空时 `sell` 目标为 0。已保存策略的
  目标和 `allow_short` 从执行配置读取，调用方不能用回测请求覆盖；
- 当前仓位和目标异号时，下一 bar 只平到 0，随后终止旧目标；不会根据原 evaluation
  自动建立反向仓位，`hold` 也不会保留或恢复它；
- 只有归零后新产生、明确、足够新鲜且与反向方向一致的 `buy`/`sell` evaluation 才
  创建新的反向目标，并在再下一 bar 按普通新开仓路径重新接受适用门控、撮合和计费；
...
当前模拟器的负仓位只用于验证目标仓位、信号和成本语义，不模拟 IBKR 保证金、借券
可用性、借券利息或强制平仓。需要评估真实空头容量时必须另行接入账户和证券级保证金
模型，不能直接把本回测的现金余额解释为 broker buying power。
...
- 可配置只允许平仓模式。
- 策略级目标持仓/资本、滚动 24 小时已实现净亏损、连续净亏损交易、滚动交易次数
  和换手预算；阈值持久化在 DuckDB，可由 Web 在策略暂停时修改；
- 成交、佣金、FX、行情或账户数据不足时失败关闭风险增加动作，但经新鲜权威持仓
  验证的严格减仓/平仓只旁路开仓限额、频率或成本绩效门控；全局交易开关、交易
  控制/紧急停止和交易日历仍然生效。
