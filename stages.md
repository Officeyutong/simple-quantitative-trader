...
- 新开仓要求行情 fresh；
- 经当前持仓验证的严格平仓允许旁路开仓行情故障门控；全局交易开关、交易控制/
  紧急停止和交易日历仍然生效；
...
- 严格平仓绕过持仓、敞口、日亏损、速率以及缺失行情/FX/PnL 等开仓限制；全局禁止
  交易、紧急停止和交易时段仍生效。
...
- 确定性事件时钟；
- 下一 bar 成交规则；
- 与自动执行一致的多头目标、空头目标和 `allow_short` 语义；
- 异号目标在下一根 bar 只平到 0，确认后终止旧目标；原 evaluation 和后续 `hold`
  都不会自动开反向仓，只有归零后新产生、明确、足够新鲜且与反向方向一致的
  `buy`/`sell` evaluation 才创建新的反向目标并按普通新开仓路径撮合、门控和计费；
- 已保存策略强制使用数据库执行配置，临时 RPC/CLI 请求保留兼容的显式目标字段；
...
- 单元测试验证信号只能在下一根 Bar 开盘成交，以及多空目标、跨零只归零和必须等待
  新鲜明确方向信号后才能反向开仓；
...
- 风险增加信号排队超过 15 分钟或来源 Bar 已不新鲜时记录为 `skipped`
  （`stale_signal`），永不补交；保护性严格减仓由独立持久目标重试，不需要新的
  evaluation，但每次尝试都必须由当前持仓证明只会降低风险。
...
- 绩效按 strategy action、order intent、execution 和 commission 归因，分开输出
  已实现毛/净 PnL、未实现 PnL、完整盯市总 PnL、换手率、胜率、已实现权益最大回撤、Sharpe、
  Sortino、每日权益和可选基准收益；缺少成交、佣金、持仓、估值行情或 FX 时不伪造
  完整总损益；
...
- 支持显式 UTC 交易 session。配置过某交易所日历后，闭市、休市或缺失当日 session
  会阻止该交易所的策略自动执行；
- 自动执行严禁使用 IBKR delayed tick，只有实时 bid/ask 才可用于风险增加订单定价；
  经当前持仓验证的严格减仓仍保留开仓输入故障的降级通道，但不能旁路全局交易开关、
  交易控制/紧急停止或交易日历；
...
- Web 的订单与策略状态页面展示这些诊断字段，便于区分未成交、部分成交、broker
  已无活动订单和明确拒绝；
- 实时绩效佣金继续以 IBKR `CommissionReport` 为准；该阶段最初按报告时当前 FX
  换算，阶段 20 的 schema 31 已改为按成交时历史 FX 换算。

成本建模边界：

- 回测与实时成本门控已共享数据库费用模型；策略绑定回测默认额外冻结并模拟当前的
  信号强度安全门槛和佣金/已完成周期毛利润门控，回测保存完整快照，实时成交后的绩效
  仍以 IBKR `CommissionReport` 为准；
...
- 达到最少交易数后，佣金/正毛利润超过上限会只阻止新的风险增加动作，不会自动关闭
  执行配置或锁住减仓和平仓。
...
## 27. 阶段 20：连续 V2 状态、策略风险预算与完整盯市

**状态：已完成（paper）**

- schema 30 将 `moving_average_cross_v2` 升级到持久状态版本 2。EMA 只以 SMA 播种
  一次，此后逐 Bar 连续递推；交叉候选、确认进度和冷却进度跨评估及重启保存；
- 多 Bar 补算只允许最后一根当前 Bar 对外发信号，较早的历史信号被消费但不补单；
- 新增数据库表 `strategy_risk_controls`，按策略维护资本、目标持仓/资本、滚动 24
  小时已实现净亏损、连续净亏损交易、滚动交易次数和换手预算；
- 新策略获得 100000 基础币种资本、1 倍目标持仓、2% 滚动净亏损、3 笔连续亏损、
  10 笔滚动交易和 10 倍滚动换手的保守默认值；Web 支持暂停后编辑、查看实时统计和
  带说明复位累计基线；
- 所有策略级阈值、开仓行情新鲜度和成本绩效门控只阻止风险增加。经 IBKR 当前持仓
  验证的严格减仓/平仓只可绕过缺失或陈旧的行情、FX、账户 PnL、敞口、频率和策略
  级开仓阈值；全局
  禁止交易、紧急停止及交易日历继续生效；
- 复位累计基线不会清空滚动 24 小时客观窗口，防止通过复位绕过亏损或换手限制；
- 绩效快照增加未实现损益、完整总净损益、成交完整性、估值完整性和告警字段。Web
  分开展示已实现、未实现和完整总损益，旧快照明确标记为仅已实现口径；
- schema 31 新增 `fx_rate_history`。已实现损益和佣金按成交时已知的新鲜汇率折算，
  不再随报告生成时的当前汇率漂移；策略绩效页可先完整对账，再通过持久化下载队列
  回补 IBKR 1 分钟 FX MIDPOINT。Bar 仅在收盘时刻成为可用报价，无法恢复的成交明细
  继续失败关闭；同类告警汇总展示，逐笔明细默认折叠并限制响应数量；
- schema 32 保存 IBKR `completed_status`，即使 completed-order 的普通状态仍显示
  `Submitted`，`Filled Size` 也能修正本地终态及成交数量；对账不会把本地终态降级，
  本地 execution 已覆盖请求数量时强制恢复为 `Filled`；
- schema 33 为 IBKR 持仓订阅租约增加 `subscription_id`。Started、Position、
  Completed、Heartbeat 和 Ended 只影响当前代际，旧订阅迟到事件不能清零或覆盖新
  快照；连接失效会终止 heartbeat 并使租约过期；
- schema 34 为策略资本保存明确币种。旧版无币种预算不会被当前
  `risk.base_currency` 猜测，而是失败关闭新开仓，要求暂停策略并在 Web 重新保存；
- schema 35 将完整持仓快照时间与 heartbeat 租约时间分离。本地成交已记录但
  `positions_current` 尚未追平时不再领取新的目标仓位动作；空仓由新完成的完整快照
  证明，heartbeat 不能伪造该证明。手工 close-only 提交会扣除同方向未决意图及活动
  订单已占用的平仓数量，多笔订单合计不能穿越零点；
- schema 36 保存方向信号的目标仓位快照、重试调度时间和每次尝试的源 evaluation。
  普通开仓失败不追逐陈旧信号；保护性严格减仓在闭市、临时数据错误、明确拒绝或撤单
  后保留目标，活动订单结束且持仓流追平后以新的合成 carrier、action 和幂等键持续
  重试，无需新的 evaluation 或 `hold` Bar，直到达到目标或被新方向取代；
- schema 37 收紧跨零与 V2 恢复语义：跨零旧 desired target 只获准归零，持仓流确认
  后立即终止，不会依据原信号自动执行第二腿。只有随后新产生、明确、足够新鲜且与
  反向方向一致的 `buy`/`sell` evaluation 才能重新通过全部开仓门控；回测遵守同一
  规则。`moving_average_cross_v2` 同时升级到状态版本 3，迁移会取消其活动目标、
  重置旧状态、暂停运行中的 V2 并禁用自动执行；状态刚重建时只允许保护性平仓，禁止
  立即新开仓，必须经人工核对并等待后续正常的新鲜方向确认；
- 自动执行订单携带 action/leg/source/target provenance；order intent 落库前在同一
  Storage 临界区重新校验 active target、配置和最新持仓到目标的方向/数量。认领后持仓
  变化不会沿用旧数量；订单类型、盘前盘后语义和限价形态也必须与持久配置一致。最终
  授权到 IBKR 返回结果期间会串行化策略评估、启停和执行配置修改。目标被取代、取消、
  放弃、过期、完成或因旧 schema 根本没有 desired 行时，已知的活动 broker order 会
  先对账再批量撤销；单笔失败不阻塞其他候选。`unknown` intent 继续阻断并等待对账，
  不自动发送补偿反单；
- 风险与绩效历史重放根据 action leg 当时持久化的负目标判断是否授权开空，不再用
  当前 `allow_short` 反向改写旧成交；FX 最新值拒绝乱序回退和未来时间戳，正向报价
  过期时会继续尝试新鲜反向报价。
